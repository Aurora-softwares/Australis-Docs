...
4. **Combine** — `bootloader.bin` and `kernel.bin` are concatenated into `iso/Australis-OS.bin`.
5. **Create the disk image** — A 2 MB raw disk image is created at `iso/Australis-OS-disk-2MB.img` via PowerShell, then the OS image is written to its start.
...
| `iso/Australis-OS.bin`            | Combined OS image                    |
